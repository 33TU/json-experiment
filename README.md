...
The benchmarks compare `MarshalAppend`, `Marshal`, `encoding/json`, and
`sonic.ConfigFastest`. `MarshalAppend` reuses caller-provided destination
capacity, while the other marshal APIs return an owned byte slice.

![Marshal benchmark comparison](assets/benchmark.svg)

```sh
go test -benchmem -run='^$' -bench='^(BenchmarkMarshalMapInt|BenchmarkMarshalMapIntSlice|BenchmarkMarshalMapAny|BenchmarkMarshalIntSlice|BenchmarkMarshalFloat32|BenchmarkMarshalFloat64|BenchmarkMarshalStruct)$'
```
