...
func createArrayMarshalFn(typ reflect.Type) marshalFn {
	...
	return createArrayDefaultMarshalFn(typ, false)
}

func createArrayDefaultMarshalFn(typ reflect.Type, addressable bool) marshalFn {
	arrayLen := typ.Len()
	if arrayLen == 0 {
		return func(dst []byte, _ unsafe.Pointer, _ MarshalFlags) ([]byte, error) {
			return append(dst, "[]"...), nil
	...
	if addressable && (!elemIsMap || hasPointerOnlyMarshaler(elemType)) {
