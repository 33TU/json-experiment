...
// AppendStringBoolSliceMapHTML appends the JSON representation of values to dst.
func AppendStringBoolSliceMapHTML(dst []byte, values map[string][]bool) []byte {
	if values == nil {
		return AppendNull(dst)
	}
	if len(values) == 0 {
...
// AppendStringFloat32SliceMapHTML appends the JSON representation of values to dst.
// It returns an error if any value is NaN or infinite.
func AppendStringFloat32SliceMapHTML(dst []byte, values map[string][]float32) ([]byte, error) {
	if values == nil {
		return AppendNull(dst), nil
	}
	if len(values) == 0 {
...
// AppendStringFloat64SliceMapHTML appends the JSON representation of values to dst.
// It returns an error if any value is NaN or infinite.
func AppendStringFloat64SliceMapHTML(dst []byte, values map[string][]float64) ([]byte, error) {
	if values == nil {
		return AppendNull(dst), nil
	}
	if len(values) == 0 {
...
// AppendStringStringSliceMapHTML appends the JSON representation of values to dst.
func AppendStringStringSliceMapHTML(dst []byte, values map[string][]string) []byte {
	if values == nil {
		return AppendNull(dst)
	}
	if len(values) == 0 {
	...
	for key, value := range values {
		dst = AppendStringHTML(dst, key)
		dst = append(dst, ':')
		dst = AppendStringSliceHTML(dst, value)
		dst = append(dst, ',')
	}
	dst[len(dst)-1] = '}'
